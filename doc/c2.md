...
### 2.6 Estrutura do Ficheiro `openapi.yaml`
...
- **`servers:`**  
  ...
  ```
  ### 2.6.1 Estrutura das Secções `paths` e `components` no OpenAPI
...
### 2.6.2 Importância da Documentação OpenAPI
...
### 2.6.3 Exemplo de Definição YAML – Listar Clientes
...
```
### 2.6.4 Exemplo de Filtros YAML – Listar Veículos com Query Parameters
...
```
### 2.6.5 Exemplo YAML – Schema da Relação Favoritos (N:N)
